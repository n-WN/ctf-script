...
def create_watermark(watermark_text: str, output_path: str, 
    ...
    if density == "auto" and len(watermark_text) < 20:
        ...
        # 背景平铺 - 更小更淡的水印
        c.setFillColor(Color(*color, alpha=opacity/2))
        c.setFont(font_name, font_size//2)
        
        # 随机分布的背景水印
        for i in range(8):  # 8个背景水印
            x = random.randint(50, int(width-50))
            y = random.randint(50, int(height-50))
            rotation = angle + random.randint(-15, 15)  # 角度变化
            size_factor = random.uniform(0.7, 1.3)  # 大小变化
            
            c.saveState()
            c.translate(x, y)
            c.rotate(rotation)
            c.setFont(font_name, int(font_size//2 * size_factor))
            c.setFillColor(Color(*color, alpha=opacity/2 * random.uniform(0.5, 1.0)))
    ...
    # 如果文本太长，考虑调整字体大小
    max_width = h_spacing * 0.8  # 留出20%的边距
    if text_width > max_width and density not in ["low", "auto"]:
        # 调整字体大小以适应间距
        adjusted_font_size = int(font_size * max_width / text_width)
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # 添加随机变化使布局更自然
            if density in ["auto", "medium", "high"]:
                # 位置偏移
                x += random.randint(-20, 20)
                y += random.randint(-20, 20)
                # 角度变化
                rotation = angle + random.randint(-10, 10)
                # 大小变化
                size_variation = random.uniform(0.8, 1.2)
                # 透明度变化
                opacity_variation = random.uniform(0.7, 1.0)
            else:
                rotation = angle
                size_variation = 1.0
